...
import { Upload, Save, Loader2, MapPin, Crosshair, User, Home, Trash2, Droplets, Image as ImageIcon } from 'lucide-react';

interface DashboardProps {
  onSuccess: () => void;
}

export const Dashboard: React.FC<DashboardProps> = ({ onSuccess }) => {
  ...
  useEffect(() => {
    ...
    const initialLat = lat || 19.302052;
    const initialLng = lng || 97.965449;

    const map = L.map('map-container').setView([initialLat, initialLng], 15);
    mapRef.current = map;
  ...
  const onSubmit = async (data: WasteRecord) => {
    ...
    try {
      ...
      const isDuplicate = existingRecords.some(r => r.fullName === data.fullName && r.id !== data.id); 
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <label className={`flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-2xl cursor-pointer transition-all ${selectedImage ? 'border-emerald-500 bg-emerald-50' : 'border-gray-300 bg-gray-50 hover:bg-gray-100'}`}>
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="mb-2 text-sm font-semibold text-gray-600">
                        {selectedImage ? selectedImage.name : 'คลิกเพื่ออัปโหลดรูปภาพ'}
                    </p>
        ...
        <div className="sticky bottom-4 z-20">
             <button
                ...
                className="w-full shadow-2xl shadow-emerald-200 flex items-center justify-center space-x-2 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-700 hover:to-teal-600 text-white font-bold py-4 px-6 rounded-2xl transform transition-all duration-300 hover:scale-[1.02] active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
            >
                {isSubmitting ? <Loader2 className="animate-spin" /> : <Save />}
                <span className="text-lg">บันทึกข้อมูลเข้าสู่ระบบ</span>
            </button>
        </div>
